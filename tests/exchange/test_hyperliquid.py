...
def test_hyperliquid_dry_run_liquidation_price(default_conf, mocker):
    """测试Hyperliquid交易所的模拟交易清算价格计算
    验证dry_run_liquidation_price()计算的清算价格是否接近CCXT返回的清算价格
    测试不同交易对（价格高低不同）、不同杠杆、多空方向的情况
    """
    markets = {
...
def test_hyperliquid__lev_prep(default_conf, mocker):
    ...
    # 期货模式下应调用set_margin_mode，杠杆应向下取整
...
def test_hyperliquid_fetch_order(default_conf_usdt, mocker):
    ...
    # 验证平均价格是否为加权平均值 ((1000*3 + 3000*1)/(3+1) = 1500)
