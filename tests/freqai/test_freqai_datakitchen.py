...
def test_create_fulltimerange_incorrect_backtest_period(mocker, freqai_conf):
    dk = get_patched_data_kitchen(mocker, freqai_conf)
    with pytest.raises(OperationalException, match=r"backtest_period_days 必须是整数"):
        dk.create_fulltimerange("20220101-20220201", 0.5)
    with pytest.raises(OperationalException, match=r"backtest_period_days 必须为正数"):
...
def test_split_timerange(
    ...
    with pytest.raises(
        OperationalException, match=r"train_period_days 必须是大于0的整数。"
    ):
