...
def test_ohlcv_to_dataframe(ohlcv_history_list, caplog):
    ...
    assert log_has("正在将K线（OHLCV）数据转换为UNITTEST/BTC交易对的数据框。", caplog)


def test_trades_to_ohlcv(trades_history_df, caplog):
    """测试交易数据转换为OHLCV数据的功能"""
    caplog.set_level(logging.DEBUG)
    with pytest.raises(ValueError, match="交易列表为空。"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("1d", 20_000, 5, 5, "2020-01-01 00:00:00", "2020-01-05 00:00:00", "星期日"),
        ("7d", 20_000, 37, 6, "2020-01-06 00:00:00", "2020-02-10 00:00:00", "星期一"),
        ("1w", 20_000, 37, 6, "2020-01-06 00:00:00", "2020-02-10 00:00:00", "星期一"),
...
def test_ohlcv_fill_up_missing_data(testdatadir, caplog):
    ...
    assert log_has_re(
        f"UNITTEST/BTC的1m数据缺失填充：之前：{len(data)} - 之后：{len(data2)}.*",
        caplog,
    )
...
def test_ohlcv_fill_up_missing_data2(caplog):
    ...
    assert log_has_re(
        f"UNITTEST/BTC的{timeframe}数据缺失填充：之前："
        f"{len(data)} - 之后：{len(data2)}.*",
        caplog,
    )
...
def test_ohlcv_to_dataframe_1M():
    """测试月度时间框架下OHLCV数据转换为数据框的功能"""
    # 2019-09-01至2023-07-01的月度ticks
    ticks = [
...
def test_ohlcv_drop_incomplete(caplog):
    ...
    assert len(data) == 4
    assert not log_has("删除最后一根K线", caplog)
    ...
    assert log_has("删除最后一根K线", caplog)
...
def test_convert_trades_to_ohlcv(testdatadir, tmp_path, caplog):
    ...
    assert log_has("删除XRP/ETH交易对1m间隔的现有数据。", caplog)
    # 加载新数据
