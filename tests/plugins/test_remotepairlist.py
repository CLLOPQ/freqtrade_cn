...
def test_fetch_pairlist_mock_response_html(mocker, rpl_config):
    ...
    with pytest.raises(OperationalException, match="RemotePairList 不是 JSON 类型。"):
...
def test_fetch_pairlist_timeout_keep_last_pairlist(mocker, rpl_config, caplog):
    ...
    assert log_has(f"错误：无法从以下地址获取交易对列表：{pairlist_url}", caplog)
    assert log_has("保留最后获取的交易对列表", caplog)
...
def test_remote_pairlist_init_no_pairlist_url(mocker, rpl_config):
    ...
    with pytest.raises(
        ...
        match=r"`pairlist_url` 未指定。请检查您的配置中的 \"pairlist.config.pairlist_url\"",
    ):
...
def test_remote_pairlist_init_wrong_mode(mocker, rpl_config):
    ...
    with pytest.raises(
        OperationalException,
        match=r'`mode` 配置不正确。支持的模式有 "whitelist","blacklist"',
    ):
        get_patched_freqtradebot(mocker, rpl_config)
    ...
    with pytest.raises(
        OperationalException, match=r"黑名单模式的 RemotePairList 不能是第一个交易对列表处理器。"
    ):
        get_patched_freqtradebot(mocker, rpl_config)
...
def test_remote_pairlist_init_wrong_proc_mode(mocker, rpl_config):
    ...
    with pytest.raises(
        OperationalException,
        match=r'`processing_mode` 配置不正确。支持的模式有 "filter","append"',
    ):
        get_patched_freqtradebot(mocker, rpl_config)
...
def test_remote_pairlist_blacklist(mocker, rpl_config, caplog, markets, tickers):
    ...
    assert log_has(f"黑名单 - 过滤掉的交易对：{pairs}", caplog)
...
def test_remote_pairlist_whitelist(mocker, rpl_config, processing_mode, markets, tickers):
    ...
    whitelist = remote_pairlist.filter_pairlist(rpl_config["exchange"]["pair_whitelist"], {})
    # 如果处理模式是"filter"，结果为["XRP/USDT"]；如果是"append"，结果为["ETH/USDT", "XRP/USDT"]
