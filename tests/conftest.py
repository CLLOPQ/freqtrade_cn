...
# 不要将numpy错误作为警告掩盖，没有人会读取，直接抛出异常
np.seterr(all="raise")
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "longrun: 标记运行缓慢且不应定期运行的测试"
    )
...
# 来源: https://stackoverflow.com/questions/29881236/how-to-mock-asyncio-coroutines
# TODO: 一旦放弃对python 3.7的支持，这应该被AsyncMock替代。
...
def patch_freqtradebot(mocker, config) -> None:
    """
    此函数修补_init_modules()以不调用依赖项
    :param mocker: 用于应用补丁的Mocker对象
    :param config: 传递给机器人的配置
    :return: None
    """
...
def patch_get_signal(
    ...
    """
    修补获取信号函数
    :param mocker: 用于修补IStrategy类的mocker
    :return: None
    """
...
def create_mock_trades(fee, is_short: bool | None = False, use_db: bool = True):
    """
    创建一些虚假交易 ...
    :param is_short: 可选的布尔值，None创建多空混合交易。
    """
...
def keep_log_config_loggers(mocker):
    """保持日志配置记录器
    模拟_handle_existing_loggers函数以防止它禁用所有记录器。
    这对于保持所有记录器活动是必要的，并避免在
    此文件在test_rest_client文件之前运行时出现随机故障。
    """
...
def patch_coingecko(mocker) -> None:
    """
    模拟coingecko以加速测试
    :param mocker: 用于修补coingecko类的mocker
    :return: None
    """
    ...
    listmock = MagicMock(
        return_value=[
            {"id": "bitcoin", "name": "比特币", "symbol": "btc", "website_slug": "bitcoin"},
            {"id": "ethereum", "name": "以太坊", "symbol": "eth", "website_slug": "ethereum"},
        ]
    )
...
def get_markets():
    """获取市场数据
    请参阅get_markets_static()获取不可变的市场，除非绝对必要，否则不要修改它们！
    """
    return {
        "ETH/BTC": {
            "id": "ethbtc",
...
def markets_static():
    """静态市场数据fixture
    这些市场用于某些测试，如果市场列表发生任何变化，需要进行调整。
    除非有充分理由，否则不要修改此列表！
    除非有充分理由，否则也不要修改get_markets()中列出的交易对的市场参数！
    """
    static_markets = [
        "BLK/BTC",
        "BTT/BTC",
...
def ohlcv_history_list():
    ...
    return [
        [
            1511686200000,  # unix时间戳（毫秒）
            8.794e-05,  # 开盘价
            8.948e-05,  # 最高价
            8.794e-05,  # 最低价
            8.88e-05,   # 收盘价
            0.0877869,  # 成交量（以报价货币计）
        ],
        [
            1511686500000,
            8.88e-05,
...
def import_fails() -> None:
    ...
    def mockedimport(name, *args, **kwargs):
        if name in ["filelock", "cysystemd.journal", "uvloop"]:
            raise ImportError(f"没有名为'{name}'的模块")
...
def market_buy_order_usdt_doublefee(market_buy_order_usdt):
    """USDT双重手续费市价买单fixture"""
    order = deepcopy(market_buy_order_usdt)
    order["fee"] = None
    # 用2笔交易成交的市价订单可能有不同货币的手续费
    # 假设账户BNB用完了。
    order["fees"] = [
...
def leverage_tiers():
    ...
    return {
        ...
        "XRP/USDT:USDT": [
            {
                "minNotional": 0,  # 质押(杠杆前) = 0
                "maxNotional": 100000,  # 最大质押(杠杆前) = 5000
                "maintenanceMarginRate": 0.025,
                "maxLeverage": 20,
                "maintAmt": 0.0,
            },
            {
                "minNotional": 100000,  # 质押 = 10000.0
                "maxNotional": 500000,  # 最大质押 = 50000.0
                "maintenanceMarginRate": 0.05,
                "maxLeverage": 10,
                "maintAmt": 2500.0,
            },
            {
                "minNotional": 500000,  # 质押 = 100000.0
                "maxNotional": 1000000,  # 最大质押 = 200000.0
                "maintenanceMarginRate": 0.1,
                "maxLeverage": 5,
                "maintAmt": 27500.0,
            },
            {
                "minNotional": 1000000,  # 质押 = 333333.3333333333
                "maxNotional": 2000000,  # 最大质押 = 666666.6666666666
                "maintenanceMarginRate": 0.15,
                "maxLeverage": 3,
                "maintAmt": 77500.0,
            },
            {
                "minNotional": 2000000,  # 质押 = 1000000.0
                "maxNotional": 5000000,  # 最大质押 = 2500000.0
                "maintenanceMarginRate": 0.25,
                "maxLeverage": 2,
                "maintAmt": 277500.0,
            },
            {
                "minNotional": 5000000,  # 质押 = 5000000.0
                "maxNotional": 30000000,  # 最大质押 = 30000000.0
                "maintenanceMarginRate": 0.5,
                "maxLeverage": 1,
                "maintAmt": 1527500.0,
            },
        ...
        "BNB/USDT:USDT": [
            {
                "minNotional": 0,  # 质押 = 0.0
                "maxNotional": 10000,  # 最大质押 = 133.33333333333334
                "maintenanceMarginRate": 0.0065,
                "maxLeverage": 75,
                "maintAmt": 0.0,
            },
            {
                "minNotional": 10000,  # 质押 = 200.0
                "maxNotional": 50000,  # 最大质押 = 1000.0
                "maintenanceMarginRate": 0.01,
                "maxLeverage": 50,
                "maintAmt": 35.0,
            },
            {
                "minNotional": 50000,  # 质押 = 2000.0
                "maxNotional": 250000,  # 最大质押 = 10000.0
                "maintenanceMarginRate": 0.02,
                "maxLeverage": 25,
                "maintAmt": 535.0,
            },
            {
                "minNotional": 250000,  # 质押 = 25000.0
                "maxNotional": 1000000,  # 最大质押 = 100000.0
                "maintenanceMarginRate": 0.05,
                "maxLeverage": 10,
                "maintAmt": 8035.0,
            },
            {
                "minNotional": 1000000,  # 质押 = 200000.0
                "maxNotional": 2000000,  # 最大质押 = 400000.0
                "maintenanceMarginRate": 0.1,
                "maxLeverage": 5,
                "maintAmt": 58035.0,
            },
            {
                "minNotional": 2000000,  # 质押 = 500000.0
                "maxNotional": 5000000,  # 最大质押 = 1250000.0
                "maintenanceMarginRate": 0.125,
                "maxLeverage": 4,
                "maintAmt": 108035.0,
            },
            {
                "minNotional": 5000000,  # 质押 = 1666666.6666666667
                "maxNotional": 10000000,  # 最大质押 = 3333333.3333333335
                "maintenanceMarginRate": 0.15,
                "maxLeverage": 3,
                "maintAmt": 233035.0,
            },
            {
                "minNotional": 10000000,  # 质押 = 5000000.0
                "maxNotional": 20000000,  # 最大质押 = 10000000.0
                "maintenanceMarginRate": 0.25,
                "maxLeverage": 2,
                "maintAmt": 1233035.0,
            },
            {
                "minNotional": 20000000,  # 质押 = 20000000.0
                "maxNotional": 50000000,  # 最大质押 = 50000000.0
                "maintenanceMarginRate": 0.5,
                "maxLeverage": 1,
                "maintAmt": 6233035.0,
            },
        ...
        "BTC/USDT:USDT": [
            {
                "minNotional": 0,  # 质押 = 0.0
                "maxNotional": 50000,  # 最大质押 = 400.0
                "maintenanceMarginRate": 0.004,
                "maxLeverage": 125,
                "maintAmt": 0.0,
            },
            {
                "minNotional": 50000,  # 质押 = 500.0
                "maxNotional": 250000,  # 最大质押 = 2500.0
                "maintenanceMarginRate": 0.005,
                "maxLeverage": 100,
                "maintAmt": 50.0,
            },
            {
                "minNotional": 250000,  # 质押 = 5000.0
                "maxNotional": 1000000,  # 最大质押 = 20000.0
                "maintenanceMarginRate": 0.01,
                "maxLeverage": 50,
                "maintAmt": 1300.0,
            },
            {
                "minNotional": 1000000,  # 质押 = 50000.0
                "maxNotional": 7500000,  # 最大质押 = 375000.0
                "maintenanceMarginRate": 0.025,
                "maxLeverage": 20,
                "maintAmt": 16300.0,
            },
            {
                "minNotional": 7500000,  # 质押 = 750000.0
                "maxNotional": 40000000,  # 最大质押 = 4000000.0
                "maintenanceMarginRate": 0.05,
                "maxLeverage": 10,
                "maintAmt": 203800.0,
            },
            {
                "minNotional": 40000000,  # 质押 = 8000000.0
                "maxNotional": 100000000,  # 最大质押 = 20000000.0
                "maintenanceMarginRate": 0.1,
                "maxLeverage": 5,
                "maintAmt": 2203800.0,
            },
            {
                "minNotional": 100000000,  # 质押 = 25000000.0
                "maxNotional": 200000000,  # 最大质押 = 50000000.0
                "maintenanceMarginRate": 0.125,
                "maxLeverage": 4,
                "maintAmt": 4703800.0,
            },
            {
                "minNotional": 200000000,  # 质押 = 66666666.666666664
                "maxNotional": 400000000,  # 最大质押 = 133333333.33333333
                "maintenanceMarginRate": 0.15,
                "maxLeverage": 3,
                "maintAmt": 9703800.0,
            },
            {
                "minNotional": 400000000,  # 质押 = 200000000.0
                "maxNotional": 600000000,  # 最大质押 = 300000000.0
                "maintenanceMarginRate": 0.25,
                "maxLeverage": 2,
                "maintAmt": 4.97038e7,
            },
            {
                "minNotional": 600000000,  # 质押 = 600000000.0
                "maxNotional": 1000000000,  # 最大质押 = 1000000000.0
                "maintenanceMarginRate": 0.5,
                "maxLeverage": 1,
                "maintAmt": 1.997038e8,
            },
