...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("trades", [5], {}),
        ("trades", [5, 5], {}),  # 带偏移量
        ("trades", [5, 5, True], {}),  # 显式指定order_by_id=True
...
def test_ft_client(mocker, capsys, caplog):
    ...
    assert log_has_re(r"无法加载配置文件 .*nonexisting\.json\.", caplog)

    args = add_arguments(
        ["--config", "tests/testdata/testconfigs/main_test_config.json", "whatever"]
    )
    main_exec(args)
    assert log_has_re("命令 whatever 未定义", caplog)
...
def test_ft_client_argparsing(mocker, params, expected_args, expected_kwargs, caplog):
    ...
    if isinstance(expected_kwargs, dict):
        ...
        assert log_has_re(f"执行命令 {mocked_method} 时出错: 收到意外的 .*", caplog)
